# Zones + Patterns Contracts

_Related: [Domain: Channel](../../domain/Channel.md) • [Domain: SchedulePlan](../../domain/SchedulePlan.md) • [Domain: ScheduleDay](../../domain/ScheduleDay.md) • [Domain: Program](../../domain/Program.md) • [Scheduling Policies](../../domain/SchedulingPolicies.md)_

## Purpose

This document defines **testable behavioral contracts** for the Zones + Patterns scheduling model. These contracts ensure that the scheduling engine correctly implements the core policies and behaviors that guarantee EPG accuracy, ad math consistency, and predictable schedule generation.
...
- **SchedulePlan** - Defines Zones (time windows) and Patterns (ordered lists of Program references)
- **Zone** - Named time windows within the programming day that reference Patterns
- **Pattern** - Ordered lists of Program references that repeat to fill Zone windows
- **Program** - Catalog entities (series, movie, block, composite) referenced by Patterns
...
**Behavior:**
- When Zones expand their Patterns across time windows, each Program reference starts at the next valid grid boundary
...
### C-PAT-01: Pattern Repeats to Fill Zone Window

**Contract:** Patterns MUST repeat across their Zone's active window until the Zone is full, snapping to Channel grid boundaries.

**Behavior:**
- When a Zone's Pattern is expanded, the ordered list of Program references repeats cyclically
- The Pattern continues repeating until the Zone's declared end time is reached
- Each repetition snaps to the next valid grid boundary
- If a Pattern does not fully fill the Zone, under-filled time becomes avails

**Test Assertions:**
- Given a Zone `00:00-06:00` with Pattern `["A", "B"]` on a 30-minute grid:
  - Program A starts at 00:00
  - Program B starts at 00:30 (or next grid boundary after A)
  - Pattern repeats: A at 01:00, B at 01:30, A at 02:00, etc.
  - Pattern continues until Zone end (06:00)
- Given a Zone `19:00-22:00` with Pattern `["Movie Block"]` that fills 1.5 hours:
  - Movie plays 19:00-20:30
  - Pattern repeats: Movie at 20:30-22:00 (or next grid boundary)
  - Zone ends at 22:00 as declared

**Related Documentation:**
- [SchedulePlan.md](../../domain/SchedulePlan.md) - Pattern section
- [SchedulingPolicies.md](../../domain/SchedulingPolicies.md) - Grid Alignment and Fixed Zone End policies
- [ScheduleDay.md](../../domain/ScheduleDay.md) - Zone and Pattern Expansion section

**Entities:** SchedulePlan, Zone, Pattern, ScheduleDay

---

### C-ZONE-01: Zone Soft-Starts After In-Flight, Snaps to Next Boundary

**Contract:** If a Zone opens while content is already playing, the Zone MUST wait until the current item ends, then start its Pattern at the next grid boundary.
...
**Contract:** Zones MUST end at their declared end time, even if the Pattern has not fully filled the Zone. Under-filled time becomes avails.

**Behavior:**
- If a Pattern does not fully fill a Zone, the Zone ends at its declared end time
...
**Test Assertions:**
- Given a Zone `20:00-22:00` (2 hours) with Pattern `["Movie Block"]` that fills 1.5 hours:
  - Movie plays 20:00-21:30
  - Zone ends at 22:00 as declared
  - 21:30-22:00 becomes avails (not filled by Pattern)
- Given a Zone `06:00-12:00` (6 hours) with Pattern `["A", "B"]` where each Program fills 1 hour:
  - Pattern repeats: A at 06:00, B at 07:00, A at 08:00, B at 09:00, A at 10:00, B at 11:00
...
**Contract:** The Electronic Program Guide (EPG) MUST reflect the actual expected start times from the compiled ScheduleDay, not the Zone declarations or Pattern definitions.
...
- EPG does not show Zone start times or Pattern definitions — only the resolved ScheduleDay entries

**Test Assertions:**
- Given a Zone `20:00-22:00` with Pattern `["Drama"]`, but content from previous Zone carries until 20:15:
  - ScheduleDay shows Drama starting at 20:30 (next grid boundary after 20:15)
  - EPG shows Drama starting at 20:30 (not 20:00)
- Given a Zone `06:00-12:00` with Pattern `["Cartoons"]`, but carry-in from previous day ends at 06:45:
...
2. **C-PAT-01** ensures Patterns fill Zones correctly
...
**Critical Rule:** These contracts are applied in order during ScheduleDay resolution. The scheduler evaluates Zones, expands Patterns, resolves Programs, and applies these contracts to generate the final immutable ScheduleDay.
...
- [Domain: SchedulePlan](../../domain/SchedulePlan.md) - Zones and Patterns model
- [Domain: ScheduleDay](../../domain/ScheduleDay.md) - Resolved schedules
- [Domain: Program](../../domain/Program.md) - Catalog entities
