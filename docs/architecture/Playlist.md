...
Playlist is part of the **Planned** layer in the scheduling architecture, positioned between [ScheduleDay](ScheduleDay.md) and [PlaylogEvent](PlaylogEvent.md). It is a **flattened, time-resolved sequence of all items from a ScheduleDay** — including programs, bumpers, commercials, and transitions — with absolute timecodes ready for playout execution.
...
Playlist is generated from [ScheduleDay](ScheduleDay.md) as part of the **Planned** layer:
...
Playlist is generated from [ScheduleDay](ScheduleDay.md). ScheduleDay contains SchedulableAssets (Programs, Assets, VirtualAssets, SyntheticAssets) placed in Zones. At playlist generation:
...
Playlist is part of the **Planned** layer and provides the **timeline input for the runtime Playlog**. [PlaylogEvent](PlaylogEvent.md) (Playlog) is the runtime execution plan aligned to the MasterClock. Playlog entries are derived from Playlist entries but aligned to the MasterClock for synchronized playout.

## Relationship to AsRun

Playlist is the pre–AsRun list. [AsRun](PlaylogEvent.md#asrun) records what actually aired during playout execution. AsRun can be compared to Playlist (via Playlog) to identify discrepancies between planned and actual playout.
