...
ScheduleDay is a **resolved, immutable daily schedule** for a specific channel and calendar date. **It is derived from [SchedulePlan](SchedulePlan.md) using Zones (time windows) and Patterns (ordered Program lists). If multiple plans are active, priority resolves overlapping Zones.** ScheduleDay is materialized 3–4 days in advance. Once generated, the schedule day is **frozen** (locked and immutable) unless force-regenerated or manually overridden by an operator.
...
ScheduleDay is a resolved, immutable daily schedule **derived from [SchedulePlan](SchedulePlan.md) using Zones (time windows) and Patterns (ordered Program lists). If multiple plans are active, priority resolves overlapping Zones.** ScheduleDay is **materialized 3–4 days in advance**. It provides the concrete schedule for a specific channel and calendar date. Once generated, the schedule day is **frozen** (locked and immutable) unless force-regenerated or manually overridden. It contains resolved asset selections with real-world wall-clock times and playback instructions. It defines:
...
9. **Schedule generation**: Create ScheduleDay record with resolved `asset_uuid` entries, wall-clock times, and playback instructions
...
11. **Playlog generation**: Generate [PlaylogEvent](PlaylogEvent.md) entries from the resolved schedule
