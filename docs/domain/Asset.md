...
- **operator_verified** (Boolean, required, default=False): Operator review flag for downstream workflow tools
  - Indicates human operator has reviewed and verified the asset
  - Separate from `approved_for_broadcast` to support staged approval workflows
...
- **last_enricher_checksum** (String(64), nullable): Hash of enricher configuration active when asset was last processed
  - Used to detect when enricher configuration changes and re-processing is needed
...
- `(collection_uuid, canonical_key_hash)` **unique** (for duplicate detection)
- `(collection_uuid, uri)` **unique** (for URI-based lookups)
...
- **Canonical Key**: Deterministic string generated by `src/retrovue/infra/canonical.py::canonical_key_for()`
  - Prefers `provider_key` or `external_id` when available (e.g., Plex rating keys)
  - Falls back to normalized URI/path with provider prefix
  - Format: `provider:collection:id` (e.g., `plex:collection:uuid:/media/movie.mp4`)
...
- **Uniqueness**: Only one Asset record per canonical identity within a collection (enforced by DB unique constraint)
...
- System tracks enricher configuration checksum in `last_enricher_checksum`
