...
def list_sources(db: Session, *, source_type: str | None = None) -> List[Dict[str, Any]]:
    ...
    for src in sources:
        try:
            enabled = db.query(Collection).filter(
                Collection.source_id == src.id,
                Collection.sync_enabled.is_(True),
            ).count()
            ingestible = db.query(Collection).filter(
                Collection.source_id == src.id,
        ...
        if getattr(src, "created_at", None):
            created_at_value = src.created_at.isoformat() if hasattr(src.created_at, "isoformat") else src.created_at
        updated_at_value = None
        if getattr(src, "updated_at", None):
            updated_at_value = src.updated_at.isoformat() if hasattr(src.updated_at, "isoformat") else src.updated_at
