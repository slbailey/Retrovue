...
def add_producer(
    ...
    try:
        ...
        if type not in type_names:
            typer.echo(f"Error: Unknown producer type '{type}'. Available types: {', '.join(type_names)}", err=True)
            raise typer.Exit(1)
    ...
    try:
        ...
        if json_output:
            ...
            for producer in producers:
                typer.echo(f"  - {producer['producer_id']}: {producer['name']} ({producer['type']}, {producer['status']})")
