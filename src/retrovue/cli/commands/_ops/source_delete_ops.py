...
def resolve_source_selector(db: Session, source_selector: str) -> list[Source]:
    ...
    if "*" in source_selector or "?" in source_selector:
        ...
        sources = (
            ...
            .filter(
                or_(
                    Source.name.like(like_pattern),
                    Source.external_id.like(like_pattern)
                )
            )
...
def delete_one_source_transactionally(db: Session, source_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if remaining_collections > 0:
            raise RuntimeError(f"{remaining_collections} orphaned collections remain for source '{source_id}'")
...
def perform_source_deletions(
    ...
    try:
        ...
        if len(results) != len(sources):
            raise RuntimeError(f"Processing incomplete: {len(results)} results for {len(sources)} sources")
