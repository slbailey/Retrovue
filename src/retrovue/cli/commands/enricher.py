...
def _validate_watermark_parameters(config: dict[str, Any]) -> None:
    ...
    if "position" in config:
        ...
        if not isinstance(position, str) or position not in valid_positions:
            raise ValueError(f"Position enrichment parameter must be one of: {', '.join(valid_positions)}")
...
def check_enricher_needs_parameters(enricher_type: str) -> bool:
    ...
    parameter_required_types = {
        "tvdb", "tmdb", "watermark", "crossfade", "llm", "file-parser", 
        "lower-third", "emergency-crawl"
    }
...
def list_enricher_types(
    ...
    dry_run: bool = typer.Option(False, "--dry-run", help="Show what would be listed without executing"),
):
    """
...
def add_enricher(
    ...
    try:
        ...
        if type not in type_names:
            typer.echo(f"Invalid enricher type '{type}'. Available types: {', '.join(type_names)}", err=True)
            raise typer.Exit(1)
        ...
        if not check_enricher_needs_parameters(type) and not config:
            typer.echo(f"Info: {type.title()} enricher requires no enrichment parameters (uses system defaults)")
        ...
        with session() as db:
            try:
                ...
                if json_output:
                    ...
                    typer.echo(f"Successfully created {enricher_domain.type} enricher: {enricher_domain.name}")
                    typer.echo(f"  ID: {enricher_domain.id}")
                    typer.echo(f"  Type: {enricher_domain.type}")
                    typer.echo(f"  Scope: {enricher_domain.scope}")
                    typer.echo(f"  Name: {enricher_domain.name}")
    ...
    try:
        ...
        if json_output:
            ...
            for enricher in enrichers:
                typer.echo(f"  {enricher['enricher_id']:<25} - {enricher['name']} ({enricher['type']})")
                if enricher['config']:
                    typer.echo(f"    Configuration: {json.dumps(enricher['config'])}")
                typer.echo(f"    Attached to: {enricher['attachments']['collections']} collections, {enricher['attachments']['channels']} channels")
...
def update_enricher(
    ...
    try:
        ...
        with session() as db:
            enricher_query = db.query(EnricherModel).filter(EnricherModel.enricher_id == enricher_id)
            ...
            if not check_enricher_needs_parameters(enricher.type) and not new_config:
                typer.echo(f"Info: {enricher.type.title()} enricher requires no enrichment parameters (uses system defaults)")
                return
            ...
            try:
                ...
                if json_output:
                    result = {
                        ...
                        "config": new_config,
                        "status": "updated",
                        "updated_at": enricher.updated_at.isoformat() if enricher.updated_at else None
                    }
                    typer.echo(json.dumps(result, indent=2))
                else:
...
def remove_enricher(
    ...
    try:
        ...
        with session() as db:
            ...
            if not force:
                typer.echo(f"Are you sure you want to remove enricher '{enricher.name}' (ID: {enricher.enricher_id})?")
            ...
            try:
                ...
                if json_output:
                    ...
                    if collection_attachments_removed > 0 or channel_attachments_removed > 0:
                        typer.echo(f"  Also removed: {collection_attachments_removed} collection attachments, {channel_attachments_removed} channel attachments")
