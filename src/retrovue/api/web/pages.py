...
async def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    from ...domain.entities import Asset, Source, SourceCollection
...
async def sources_list(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("sources_list.html", {
        "request": request,
        "sources": sources
    })
...
async def discover_libraries(request: Request, source_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return templates.TemplateResponse("collections_table.html", {
            "request": request,
            "source_id": source_id,
...
async def play_asset(
    ...
    return templates.TemplateResponse("playback_result.html", {
        "request": request,
        "message": message,
        "asset_id": asset_id
    })
...
async def update_collection(
    ...
    try:
        ...
        if mapping_pairs_list:
            success = source_service.update_collection_mapping(source_id, external_id, mapping_pairs_list)
            if not success:
        ...
        return templates.TemplateResponse("collection_row.html", {
            "request": request,
            "source_id": source_id,
            "collection": {
...
async def review_list(request: Request, db: Session = Depends(get_db)):
    ...
    for review in reviews:
        ...
        review_items.append({
            ...
            "raw_labels": getattr(asset, 'raw_labels', {}) if hasattr(asset, 'raw_labels') else {}
        })
