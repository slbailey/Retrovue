...
class MPEGTSWatchdog:
        ...
        try:
            while self._running:
                try:
                    ...
                    logger.info(f"Starting watched MPEG-TS stream (attempt #{self.restart_count + 1})")
    ...
    async def _handle_restart(self) -> None:
        ...
        logger.info(f"Restarting in {delay:.1f}s (backoff: {self._backoff_delay:.1f}s, jitter: {jitter:.1f}s)")
