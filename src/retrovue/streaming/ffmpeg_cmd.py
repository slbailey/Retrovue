        ...
        if line.startswith('file '):
            ...
            if not file_obj.exists():
                ...
                try:
                    ...
                    result["files_invalid"].append(file_path)
                    result["valid"] = False
                    result["errors"].append(f"Cannot access file (line {line_num}): {file_path} - {e}")
        else:
...
def build_cmd(
    ...
    cmd.extend([
        "-nostdin", "-hide_banner", "-nostats", "-loglevel", log_level,
...
def validate_cmd_args(cmd: list[str]) -> dict[str, bool]:
    ...
    return {
        "has_global_flags": all(flag in cmd_str for flag in ["-nostdin", "-hide_banner", "-nostats", "-loglevel error"]),
        "has_concat_input": "-f concat" in cmd_str and "-safe 0" in cmd_str,
        "has_protocol_whitelist": "-protocol_whitelist" in cmd_str,
        "has_mapping": "-map 0:v:0" in cmd_str and ("-map 0:a:0?" in cmd_str or "-map 0:a:0" in cmd_str),
