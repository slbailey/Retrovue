...
def validate_ffmpeg_flags(flags: list[str]) -> list[str]:
    ...
    has_mp4_flags = any(flag.startswith("-movflags") and any(mp4_flag in flag for mp4_flag in mp4_only_flags) for flag in cleaned_flags)
    ...
    streaming_flags = [flag for flag in cleaned_flags if any(stream_flag in flag for stream_flag in ["-re", "-fflags", "-avoid_negative_ts"])]
    ...
    if invalid_combinations:
        raise ValueError(f"Invalid FFmpeg flag combinations detected: {', '.join(invalid_combinations)}")
