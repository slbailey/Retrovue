    ...
    **Critical Rule:** Do not bypass LibraryService to write Assets directly.
    ...
    def list_assets(self, status: Literal["pending", "canonical"] | None = None, include_deleted: bool = False) -> list[Asset]:
        ...
        try:
            ...
            # Log the listing
            logger.info("assets_listed", count=len(assets), status=status, include_deleted=include_deleted)
    ...
    def list_canonical_assets(self, query: str | None = None, include_deleted: bool = False) -> list[Asset]:
        """
        ...
        try:
            ...
            # Log the listing
            logger.info("canonical_assets_listed", count=len(assets), query=query, include_deleted=include_deleted)
    ...
    def list_assets_advanced(
        ...
        try:
            ...
            if series:
                ...
                query = query.filter(ProviderRef.raw['grandparentTitle'].astext.ilike(f"%{series}%"))
            ...
            logger.info("assets_listed_advanced", count=len(assets), kind=kind, series=series, season=season, q=q)
            return assets
            
        except Exception as e:
            logger.error("assets_listing_advanced_failed", error=str(e), kind=kind, series=series, season=season, q=q)
            raise
    ...
    def get_asset_by_id(self, asset_id: int, include_deleted: bool = False) -> Asset | None:
        ...
        try:
            ...
            asset = query.first()
            logger.info("asset_retrieved", asset_id=str(asset_id), found=asset is not None, include_deleted=include_deleted)
            return asset
    ...
    def get_asset_by_uuid(self, asset_uuid: UUID, include_deleted: bool = False) -> Asset | None:
        ...
        try:
            ...
            asset = query.first()
            logger.info("asset_retrieved_by_uuid", asset_uuid=str(asset_uuid), found=asset is not None, include_deleted=include_deleted)
            return asset
    ...
    def get_asset_by_source_rating_key(self, rating_key: str) -> Asset | None:
        ...
        try:
            ...
            provider_ref = session.query(ProviderRef).filter(
            ...
            logger.info("asset_retrieved_by_rating_key", rating_key=rating_key, asset_id=str(asset.id) if asset else None)
            return asset
            
        except Exception as e:
            logger.error("asset_retrieval_by_rating_key_failed", rating_key=rating_key, error=str(e))
            raise
    ...
    def list_episodes_by_series(self, series: str) -> list[Asset]:
        ...
        try:
            ...
            query = session.query(Asset).join(ProviderRef, Asset.id == ProviderRef.asset_id).filter(
    ...
    def soft_delete_asset_by_uuid(self, asset_uuid: UUID, deleted_at: datetime | None = None) -> bool:
        """
    ...
    def soft_delete_asset_by_source_rating_key(self, rating_key: str, deleted_at: datetime | None = None) -> bool:
        """
        ...
        try:
            ...
            logger.info("asset_soft_deleted_by_rating_key", rating_key=rating_key, asset_id=str(asset.id))
            return True
            
        except Exception as e:
            logger.error("asset_soft_delete_by_rating_key_failed", rating_key=rating_key, error=str(e))
            raise
    ...
    def is_asset_referenced_by_episodes(self, asset_id: int) -> bool:
        ...
        try:
            ...
            logger.info("asset_reference_check", asset_id=str(asset_id), referenced=count > 0, count=count)
            return count > 0
    ...
    def hard_delete_asset_by_uuid(self, asset_uuid: UUID, force: bool = False) -> bool:
        ...
        try:
            ...
            if not force and self.is_asset_referenced_by_episodes(asset.id):
                raise ValueError(f"Asset {asset_uuid} is referenced by episodes. Use force=True to override.")
