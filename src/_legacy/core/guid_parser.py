...
class GUIDParser:
    ...
    PROVIDER_PATTERNS = {
        ...
        'imdb': [
            r'imdb://(tt\d+)',
            r'com\.plexapp\.agents\.imdb://(tt\d+)'
        ],
        'plex': [
            r'plex://(show/[^/]+)',
            r'plex://(movie/[^/]+)'
    ...
    @classmethod
    def parse_guid(cls, guid: str) -> Optional[Tuple[str, str]]:
        """
    ...
    @classmethod
    def get_primary_guid(cls, guids: List[str]) -> Optional[str]:
        """
...
def get_show_disambiguation_key(title: str, year: Optional[int]) -> str:
    """
...
def format_show_for_display(title: str, year: Optional[int], guids: List[Tuple[str, str]] = None) -> str:
    """
