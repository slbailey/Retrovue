    ...
    def get_asset_by_uuid(asset_uuid: uuid.UUID) -> dict[str, Any] | None:
        ...
        with session() as db:
            ...
            review_items = db.execute(
                select(ReviewQueue).where(ReviewQueue.asset_id == asset.id)
            ).scalars().all()
            
            review_summary = {
                "total_items": len(review_items),
                "pending_items": len([item for item in review_items if item.status.value == "PENDING"]),
                "resolved_items": len([item for item in review_items if item.status.value == "RESOLVED"])
            }
            
            # Get markers summary
            markers = db.execute(
                select(Marker).where(Marker.asset_id == asset.id)
            ).scalars().all()
