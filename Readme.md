...
### Core Hierarchy: Plan → ScheduleDay → Playlog

**SchedulePlan** → **ScheduleDay** → **Playlog**

1. **SchedulePlan** - Operator-created plans that define programming intent for a channel and date range. Plans contain block assignments that specify what content should air at specific times.

2. **ScheduleDay** - Resolved, immutable daily schedules generated from plans. Each schedule day contains concrete asset selections with real-world wall-clock times for a specific channel and date. Schedule days are locked once generated unless force-regenerated by an operator.

3. **Playlog** - Fine-grained playout events generated from schedule days. The playlog contains specific assets, commercials, and bumpers in exact playout order with precise timestamps that drive actual broadcast execution.

### Key Concepts

**VirtualAsset** - A container for multiple assets that enables packaging and re-use of modular asset bundles. VirtualAssets can be fixed sequences (e.g., intro → episode → outro) or rule-based definitions (e.g., "3 random SpongeBob segments"). They expand to concrete asset references during ScheduleDay resolution.

**SchedulePlanLabel** - Optional UI-only labels for visual organization of block assignments (e.g., "Morning Cartoons", "Prime Time"). Labels are purely organizational and do not affect scheduling logic—they exist solely to help operators organize and visualize their schedule plans.

**Layering** - Multiple plans can be layered using Photoshop-style priority resolution, where more specific plans override generic ones. This allows operators to create base plans with override plans for special events, holidays, or seasonal programming.

**Broadcast Day Anchoring** - Schedules are anchored to each channel's `broadcast_day_start` (typically 6:00 AM local time). Schedule times are calculated as absolute offsets from 00:00 within the broadcast day, then combined with the channel's broadcast day boundary to produce final wall-clock times. This ensures schedules align correctly with the channel's operational day, which may not align with calendar midnight.
