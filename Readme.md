...
Retrovue's scheduling system follows a clear hierarchy that transforms operator intent into actual broadcast execution:

### Core Hierarchy: SchedulePlan → ScheduleDay → Playlist → PlaylogEvent → AsRun

**SchedulePlan** → **ScheduleDay** → **Playlist** → **PlaylogEvent** → **AsRun Log**

1. **SchedulePlan** - Operator-created plans that define programming intent using Zones (time windows) that hold SchedulableAssets (Programs, Assets, VirtualAssets, SyntheticAssets) directly. Plans are timeless and reusable.

2. **ScheduleDay** - Resolved, immutable daily schedules generated from plans 3–4 days in advance (EPG horizon). Contains SchedulableAssets placed in Zones with wall-clock times. SchedulableAssets remain intact — expansion to physical assets occurs at playlist generation.

3. **Playlist** - Resolved pre–AsRun list of physical assets with absolute timecodes. Generated from ScheduleDay by expanding SchedulableAssets to physical Assets. Programs expand their asset chains, and VirtualAssets expand to physical Assets.

4. **PlaylogEvent** - Runtime execution plan aligned to MasterClock. Derived from Playlist entries (may diverge for substitutions/timing corrections). Generated using rolling horizon (few hours ahead).

5. **AsRun Log** - Observed ground truth — what actually aired. Records actual playback observations for historical accuracy and audits.
...
**VirtualAsset** - SchedulableAsset type that acts as an input-driven composite. Expands to physical Assets at playlist generation, enabling modular programming blocks (e.g., branded intro → episode → outro).

**Zone** - Named time windows within the programming day that hold SchedulableAssets directly. Controls timing and duration.

**Layering** - Multiple plans can be layered using priority resolution, where more specific plans override generic ones. Higher priority plans override lower priority plans when both are active for the same date.

**Broadcast Day Anchoring** - Schedules are anchored to each channel's `programming_day_start` (typically 6:00 AM local time). Schedule times are calculated as absolute offsets from 00:00 within the broadcast day, then combined with the channel's broadcast day boundary to produce final wall-clock times.
